...
public final class ControllerFrameGame implements ActionListener {
    ...
    public void askNewGame() {
        int newGame = frameGame.getOptionPane().showConfirmDialog(frameGame, "¿Desea jugar de nuevo?");
        if (newGame == frameGame.getOptionPane().YES_OPTION) {
            frameGame.dispose();
            main.setInitialFrame(null);
            main.startGameInterfaz();
        } else {
            frameGame.getOptionPane().showMessageDialog(frameGame, "Gracias por jugar");
            frameGame.dispose();
        }
    }
